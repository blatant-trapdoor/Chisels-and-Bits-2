...
public class ChiseledBlockBaked extends BaseBakedBlockModel {
            ...
            final ModelRenderState mrs)
    {
        ...
        for ( final ArrayList<FaceRegion> src : rset )
        {
            ...
            for ( final FaceRegion region : src )
            {
                ...
                final ModelQuadLayer[] mpc = ModelUtil.getCachedFace( region.blockStateID, new Random(), myFace, myLayer.layer );
    ...
    {
        processFace(blob, visFace, mrs, rset, Direction.EAST);
        processFace(blob, visFace, mrs, rset, Direction.WEST);
    }
    ...
    private void processYFaces(
            ...
            final ArrayList<ArrayList<FaceRegion>> rset )
    {
        processFace(blob, visFace, mrs, rset, Direction.UP);
        processFace(blob, visFace, mrs, rset, Direction.DOWN);
    }
    ...
    private void processZFaces(
            ...
            final ArrayList<ArrayList<FaceRegion>> rset )
    {
        processFace(blob, visFace, mrs, rset, Direction.SOUTH);
        processFace(blob, visFace, mrs, rset, Direction.NORTH);
    }
    ...
    {
        return faceCount() >= 40; //TODO add config option
    }
