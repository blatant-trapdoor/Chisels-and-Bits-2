...
public class SelectedItemMode implements IItemMode {
    ...
    private int color;
    private SelectedItemMode(final ResourceLocation key, final boolean fluid) { this.key = key; this.fluid = fluid; }
    private SelectedItemMode(final int color) { this.color=color; }
    ...
    public String getName() { return color!=0 ? String.valueOf(color) : key==null ? "null" : key.toString(); }
    public ItemModeType getType() { return color!=0 ? ItemModeType.SELECTED_BOOKMARK : fluid ? ItemModeType.SELECTED_FLUID : ItemModeType.SELECTED_BLOCK; }
}
